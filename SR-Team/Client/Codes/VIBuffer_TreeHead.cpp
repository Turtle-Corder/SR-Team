#include "stdafx.h"
#include "Pipeline.h"
#include "..\Headers\VIBuffer_TreeHead.h"

USING(Client)

CVIBuffer_TreeHead::CVIBuffer_TreeHead(LPDIRECT3DDEVICE9 pDevice)
	: CVIBuffer(pDevice)
{
}

CVIBuffer_TreeHead::CVIBuffer_TreeHead(const CVIBuffer_TreeHead & other)
	: CVIBuffer(other)
...
HRESULT CVIBuffer_TreeHead::Setup_Component_Prototype()
{
	...
	pVertex[0].vPosition = D3DXVECTOR3(-0.3f, 1.f, -0.3f);
	pVertex[0].vUV = pVertex[0].vPosition;

	pVertex[1].vPosition = D3DXVECTOR3(0.3f, 1.f, -0.3f);
	...
	pVertex[4].vPosition = D3DXVECTOR3(-0.3f, 1.f, 0.3f);
	pVertex[4].vUV = pVertex[4].vPosition;

	pVertex[5].vPosition = D3DXVECTOR3(0.3f, 1.f, 0.3f);
	pVertex[5].vUV = pVertex[5].vPosition;

	pVertex[6].vPosition = D3DXVECTOR3(0.5f, 0.f, 0.5f);
	pVertex[6].vUV = pVertex[6].vPosition;

	pVertex[7].vPosition = D3DXVECTOR3(-0.5f, 0.f, 0.5f);
...
HRESULT CVIBuffer_TreeHead::Setup_Component(void * pArg)
{
	return S_OK;
}

void * CVIBuffer_TreeHead::Get_Vertices() const
{
	return m_pVertices;
}

void * CVIBuffer_TreeHead::Get_Indices() const
{
	return nullptr;
}

HRESULT CVIBuffer_TreeHead::Set_Transform(const _matrix * _pMatWorld, const _matrix * _pMatView, const _matrix * _pMatProj)
{
...
HRESULT CVIBuffer_TreeHead::Set_Transform(const _matrix * _pMatWorld, const CCamera * _pCamera)
{
	for (_uint i = 0; i < m_iVertexCount; ++i)
	{
		// 월드 변환
...
CVIBuffer_TreeHead * CVIBuffer_TreeHead::Create(LPDIRECT3DDEVICE9 pDevice)
{
	if (nullptr == pDevice)
		return nullptr;

	CVIBuffer_TreeHead* pInstance = new CVIBuffer_TreeHead(pDevice);
...
CComponent * CVIBuffer_TreeHead::Clone_Component(void * pArg)
{
	CVIBuffer_TreeHead* pInstance = new CVIBuffer_TreeHead(*this);
